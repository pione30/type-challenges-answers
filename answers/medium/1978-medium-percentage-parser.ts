...
type Sign = "+" | "-";
type PercentageSymbol = "%";
type Empty = "";

type SignParser<A extends string> = A extends `${Sign}${infer R}`
  ? A extends `${infer H}${infer R}`
    ? H
    : never
  : Empty;

type UnsignedPercentageParser<A extends string> =
  A extends `${infer N}${PercentageSymbol}`
    ? [N, PercentageSymbol]
    : [A, Empty];

type PercentageParser<A extends string> = A extends `${Sign}${infer R}`
  ? [SignParser<A>, ...UnsignedPercentageParser<R>]
  : [Empty, ...UnsignedPercentageParser<A>];
