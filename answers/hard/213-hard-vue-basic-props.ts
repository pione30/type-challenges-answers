...
type ValueOf<T extends { valueOf(): unknown }> = T extends
  | String
  | Boolean
  | Number
...
type Props<P extends Record<string, unknown>> = {
  [K in keyof P]: P[K] extends { type: infer T }
    ? T extends unknown[]
      ? T[number] extends abstract new (...args: any) => infer R
        ? ValueOf<R>
        : never
      : T extends abstract new (...args: any) => infer R
      ? ValueOf<R>
      : never
    : P[K] extends abstract new (...args: any) => infer R
    ? ValueOf<R>
    : any;
};

type VueBasicPropsOptions<
...
> = {
  props: P;
  data(this: Props<P>): D;
  computed: C & ThisType<Props<P> & D>;
  methods: M &
    ThisType<Props<P> & D & { [K in keyof C]: ReturnType<C[K]> } & M>;
};
...
declare function VueBasicProps<
  ...
  C extends Record<string, () => unknown>,
  M extends Record<string, () => unknown>
>(options: VueBasicPropsOptions<P, D, C, M>): any;
