...
type Curried<T extends (...args: any[]) => any> = Parameters<T> extends [
  infer First,
  ...infer Rest
]
  ? Rest extends []
    ? // TODO: The return type of this branch should be `ReturnType<T>`, but that breaks the type down to `boolean`...
      (x: First) => true
    : (x: First) => Curried<(...args: Rest) => ReturnType<T>>
  : never;
